...
class Active_account extends CI_Controller {
	...
	private $permissionName = 'report_active_account';
	...
	public function index() {
		...
		if($flag=='1') {
			...
			if(!empty($logTimeStart) && !empty($logTimeEnd)) {
				...
				$parameter['log_time_start'] = $logTimeEnd;
			}
			if(!empty($gameId)) {
			...
			switch($type) {
				case '1':
					$this->load->model('report/active_count_daily', 'active');
					break;
				case '2':
					$this->load->model('report/active_count_week', 'active');
					break;
				case '3':
					$this->load->model('report/active_count_month', 'active');
					break;
				default:
					$this->load->model('report/active_count_daily', 'active');
					break;
			 ...
			 * @novar
			 */
			$rowTotal = $this->active->getTotal($parameter);
			...
			$result = $this->active->getAllResult($parameter, $itemPerPage, $offset);
		...
		$this->render->render('report_active_account', $data);
	}
}
?>