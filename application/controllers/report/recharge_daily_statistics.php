...
class Recharge_daily_statistics extends CI_Controller {
	private $user = null;
	private $_CONFIG = null;
	private $permissionName = 'report_recharge_daily_statistics';
	...
	public function index() {
		...
		if($flag=='1') {
			...
			if(!empty($fundsType)) {
				$parameter['funds_type'] = $fundsType;
			}
			/**
			 * 
			 * 分页程序
			 * @novar
			 */
			$rowTotal = $this->overview_model->getTotal($parameter);
			...
			$result = $this->overview_model->getAllResult($parameter, $itemPerPage, $offset);
		...
		$this->render->render('report_daily_statistics', $data);
	}
}
?>