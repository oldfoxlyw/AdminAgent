...
class Check_user extends CI_Model {
	...
	public function validate() {
		...
		if(!$this->input->cookie('adminagent_user', TRUE)) {
			redirect("/message?info=CMS_USER_EXPIRED&redirect={$redirectUrl}&auto_redirect=1&auto_delay=5");
		} else {
			$cookie = $this->input->cookie('adminagent_user', TRUE);
			...
			if($query->num_rows() > 0) {
				...
				if($checkCode != $json->check_code) {
					redirect("/message?info=CMS_USER_CHECKCODE_INVALID&redirect={$redirectUrl}");
				} else {
	...
	public function ip() {
		...
		if($enabled) {
			...
			if(!in_array($currentIp, $allowIpList)) {
				$redirectUrl = urlencode($this->config->item('root_path') . 'login');
				redirect("/message?info=CMS_IP_INVALID&redirect={$redirectUrl}&auto_redirect=1&auto_delay=10");
			}
		}
	}
	...
	public function checkDefaultWeb() {
		...
		if(!$this->input->cookie('adminagent_default_web', TRUE)) {
			$redirectUrl = urlencode($this->config->item('root_path') . 'content/webs');
			redirect("/message?info=CMS_DEFAULT_WEB_INVALID&redirect={$redirectUrl}");
		} else {
			return $this->input->cookie('adminagent_default_web', TRUE);
		}
	}
}
?>