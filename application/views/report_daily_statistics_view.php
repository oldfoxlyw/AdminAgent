        ...
        <div class="pad20">
        	...
        	<form action="" method="post" enctype="application/x-www-form-urlencoded">
            	<fieldset>
                    ...
                    <p>
                    	<label for="funds_type">充值渠道</label>
                        <select id="funds_type" name="funds_type">
                        	<option value="0"<?php if(empty($funds_type)): ?>selected="selected"<?php endif; ?>>全部</option>
                        	<option value="1"<?php if($funds_type=='1'): ?>selected="selected"<?php endif; ?>>App Store</option>
                        </select>
                    </p>
                    <p>
        	...
        	<table class="fullwidth" cellpadding="0" cellspacing="0" border="0">
                ...
                <?php if(!empty($result)): ?>
                    ...
                    <tr<?php if($i%2==0): ?> class="odd"<?php endif; ?>>
                        ...
                        <td><?php echo intval($result[$i]->log_checkin_count) / 100; ?></td>
                        <td><?php echo $result[$i]->log_item_count; ?></td>
                        <td><?php echo $result[$i]->log_pay_count; ?></td>
                        <td><?php echo $result[$i]->log_payment_count; ?></td>
                        <td>
						<?php
						if(intval($result[$i]->log_active_count) == 0) {
							echo '0.00';
						} else {
                        	echo intval($result[$i]->log_checkin_count) / intval($result[$i]->log_active_count) / 100;
						}
						?>
                        </td>
                    </tr>
